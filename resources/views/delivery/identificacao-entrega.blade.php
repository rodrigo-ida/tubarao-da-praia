...
if (empty(Session::get('data')) || Session::get('data') == null) {
    header('location: /delivery/identificacao');
}
...
<script>
    ...
    if ($URI == '/delivery/identificacao/entrega') {
    ...
    $(document).ready(function() {
        ...
        $('#scroll-forms').append(getEndForm(
            ...
            $data2['nome'] = $data['nome'];

            $data2['cep'] = $data['cep'];

            $data2['logradouro'] = $data['logradouro'];

            $data2['numero'] = $data['numero'];

            $data2['cidade'] = $data['cidade'];

            $data2['bairro'] = $data['bairro'];
