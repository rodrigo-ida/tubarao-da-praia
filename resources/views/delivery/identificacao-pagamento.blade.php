...
<div class="login clearfix">
    ...
    @if(Session::get('msg'))
    <div class="alert alert-danger"><strong>{{ Session::get('msg') }}</strong></div>
    @endif
    <div id="scroll-forms">
...
<script>
    $(document).ready(function() {
        ...
        $('#scroll-forms').append(getEndForm(
            ...
            ?>,
            '{{ csrf_token() }}', 'pagamento', <?php echo json_encode($data); ?>));

        var payments = <?php echo $payments; ?>
        ...
        $('select#select-forma-pagamento2').append(html);

        var scroll = jQuery('#form-enviar-pagamento').offset();
    ...
    $(document.body).on('click', '#prosseguir-finalizacao', function() {
        if ($('#select-forma-pagamento2 option:selected').text() === ' Dinheiro ') {
            $('input[name="obs_payment"]').prop('required', true);
        } else {
            $('input[name="obs_payment"]').prop('required', false);
        }

        if ($('#select-forma-pagamento2 option:selected').text() === ' Dinheiro ' && $('input[name="obs_payment"]').val().length == 0) {
            alert('digite o valor para troco!');
            return;
        }


        if (!$('#cpf_nota').css('border-color', 'green') && $('#cpf-sim').is(':checked') && $('#cpf-sim').val() == 'sim') {
            alert('Verifique seu cpf e tente novamente');
        }

        if ($('#select-forma-pagamento2 option:selected')) {
            $('#form-enviar-pagamento').submit();
        } else {
    ...
    $(document).ready(function() {
        ...
        $('#select-forma-pagamento2').change(function() {
            var val = $(this).text();

            if (val.match(/eRede/)) {
                $('#erede-form').css('display', 'block');
                $('#erede-card-number').mask('0000 0000 0000 0000');
                $('#erede-exp').mask('99/9999');
            }
        });
    });
</script>
